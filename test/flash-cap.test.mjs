...
// that makes it safe to ship is the ≤50/sec rate limit (WCAG 3.0 internal). These tests
...
test('cap is pinned to the WCAG 3.0 internal value of 50 flashes/sec', () => {
  ...
  // test to match — restore the value to 50. There is no standard permitting more.
...
test('flashAllowed enforces the floor between flash starts', () => {
  ...
  assert.equal(flashAllowed(0.01, 0), false);           // too soon (<0.02s) → dropped
  assert.equal(flashAllowed(0.0199, 0), false);         // still just under the floor
  assert.equal(flashAllowed(0.02, 0), true);            // past the floor → allowed
});
...
// more than 50 flashes START in any rolling one-second window.
