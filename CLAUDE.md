...
**Couple-photo secrecy gate (`revealCouple`, time-based).** The couple's gallery photos stay secret until `coupleRevealOffsetHours` (default 5) **after** `wedding.weddingTsUTC`. Also **leak-proof by construction** — and, unlike `revealDate`, the flag is *computed from the build clock*, not a hand-set boolean: `computeRevealCouple(wedding.weddingTsUTC)` returns `now >= Date.UTC(...weddingTsUTC) + 5h`. While hidden, build.js (1) excludes `assets/photos/` from `dist` via `copyFilter`, and (2) emits a **gated `GALLERY`** into `couple.mjs` with every `src`/`alt` stripped (only the non-identifying grid-sizing `cls` survives) — so no photo file, filename, or caption ships. `config.js` re-exports `GALLERY`/`REVEAL_COUPLE` from `couple.mjs` (the `gallery` source-of-truth with captions lives in `site.config.mjs`, never shipped verbatim). `gallery.js` renders a single veiled "unveiled after the celebration" panel when every item lacks a `src`. The painted portrait `art-couple.jpg` is intentionally **not** gated. Because reveal is build-time, auto-revealing at wedding+5h needs a **scheduled Netlify rebuild** at/after that moment (build hook + scheduled function/cron); until a build runs past the threshold the photos stay excluded. Override for testing/publish with `REVEAL_COUPLE=true|false node build.js`. Helpers `computeRevealCouple`/`gateGallery` are pure + unit-tested.
