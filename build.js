...
export function resolveRevealDate(env = process.env.REVEAL_DATE, fallback = revealDate) {
...
function runBuild() {
  ...
  const htmlTokens = buildHtmlTokens(names, reveal, theme, fromGroomSide, variant);
  ...
  const coupleModule = [
    ...
    'export const WEDDING_TS = ' + (reveal ? 'Date.UTC(' + wedding.weddingTsUTC.join(', ') + ')' : 'null') + ';',
